      ...
      <div className="flex flex-row items-center justify-center gap-8 py-2 w-full">
        ...
        <div className="relative h-36 w-36 flex items-center justify-center shrink-0">
          <svg viewBox="0 0 120 120" className="w-full h-full transform -rotate-90">
            {/* Background ring */}
            <circle cx="60" cy="60" r={r} fill="transparent" stroke="#F1F5F9" strokeWidth="11" />
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <span className="text-3xl font-black text-slate-900 leading-none">{counts.total}</span>
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Total</span>
          </div>
        </div>

        {/* Legend listing */}
        <div className="space-y-3.5 text-xs font-bold text-slate-700 w-full max-w-[180px]">
          {slices.map((slice, i) => {
            ...
            return (
              <div key={i} className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-slate-600 font-semibold">{slice.name}</span>
                </div>
                <div className="text-right text-slate-900 font-extrabold">
                  {slice.count} <span className="text-slate-400 font-medium text-[10px]">({percentage}%)</span>
                </div>
              </div>
            );
          })}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="glass rounded-3xl p-5 border border-slate-150 shadow-sm flex items-center justify-between bg-white">
          <div className="space-y-2">
            ...
            <span className="text-10px text-blue-600 font-bold block">↗ 12% from last month</span>
          </div>
        ...
        <div className="glass rounded-3xl p-5 border border-slate-150 shadow-sm flex items-center justify-between bg-white">
          <div className="space-y-2">
            ...
            <h3 className="text-3xl font-black text-slate-900 leading-none">{counts.new}</h3>
            <span className="text-10px text-amber-500 font-bold block">↗ 8% from last month</span>
          </div>
        ...
        <div className="glass rounded-3xl p-5 border border-slate-150 shadow-sm flex items-center justify-between bg-white">
          <div className="space-y-2">
            ...
            <h3 className="text-3xl font-black text-slate-900 leading-none">{counts.completed}</h3>
            <span className="text-10px text-emerald-500 font-bold block">↗ 18% from last month</span>
          </div>
        ...
        <div className="glass rounded-3xl p-5 border border-slate-150 shadow-sm flex items-center justify-between bg-white">
          <div className="space-y-2">
            ...
            <h3 className="text-3xl font-black text-slate-900 leading-none">{counts.users}</h3>
            <span className="text-10px text-purple-600 font-bold block">↗ 10% from last month</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 glass rounded-3xl p-6 border border-slate-150 shadow-sm bg-white space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-sm font-black text-slate-900">Orders Overview</h3>
            <select className="bg-slate-50 border border-slate-150 rounded-xl px-3 py-1.5 text-[10px] font-bold text-slate-600 outline-none focus:border-blue-500">
        ...
        <div className="lg:col-span-4 glass rounded-3xl p-6 border border-slate-150 shadow-sm bg-white flex flex-col justify-between">
          <h3 className="text-sm font-black text-slate-900 pb-2 border-b border-slate-100">Order Status</h3>
      ...
      <div className="glass rounded-3xl border border-slate-150 shadow-sm bg-white overflow-hidden space-y-4">
        ...
        <div className="px-6 pt-5 pb-2 flex justify-between items-center">
          ...
          <Link 
            to="/admin/orders" 
            className="flex items-center gap-1.5 rounded-xl border border-blue-200 px-4 py-2 text-[10px] font-bold text-blue-600 hover:bg-blue-50 transition-colors"
          >
            View All Orders
        ...
        {recentOrders.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-y border-slate-100 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                ...
                {recentOrders.map((order) => (
                  <tr key={order._id} className="hover:bg-slate-50/40 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <div className="font-extrabold text-slate-900 leading-tight">{order.userId?.name}</div>
                      <div className="text-[10px] text-slate-500 font-medium mt-0.5">{order.userId?.email}</div>
                    </td>
                    ...
                    <td className="py-4 px-6 max-w-xs">
                      <div className="flex items-center gap-2 truncate">
                        ...
                        <span className="truncate text-slate-800 font-bold">{order.documentId?.originalName}</span>
                      </div>
                    </td>

                    {/* Pages */}
                    <td className="py-4 px-6 text-center font-bold text-slate-900">{order.pages}</td>

                    {/* Copies */}
                    <td className="py-4 px-6 text-center font-bold text-slate-900">{order.copies}</td>
