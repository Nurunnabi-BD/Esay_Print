    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-dark-400">Review, track, and details of all your placed document prints.</p>
        </div>
      </div>

      {/* Filters and search block */}
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white dark:bg-dark-950 border border-slate-200 dark:border-dark-800 p-4 rounded-2xl shadow-sm">
            ...
            <div
              ...
              className={`px-4 py-2 rounded-xl text-xs font-semibold shrink-0 transition-all cursor-pointer ${
                filter === status
                  ...
                  : 'bg-slate-50 dark:bg-dark-900 border border-slate-200 dark:border-dark-800 text-slate-700 dark:text-dark-300 hover:bg-blue-50 dark:hover:bg-dark-800 hover:text-blue-600 dark:hover:text-blue-400'
              }`}
            >
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="block w-full rounded-xl bg-slate-50 dark:bg-dark-900 border border-slate-200 dark:border-dark-800 py-2.5 pl-9 pr-3 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-dark-500 focus:border-blue-600 focus:outline-none text-sm transition-colors"
          />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-dark-950 border border-slate-200 dark:border-dark-800 rounded-3xl py-20 flex flex-col items-center justify-center text-center p-6 shadow-sm">
          ...
          <Link
            to="/dashboard"
            className="mt-6 inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-blue-700 transition-colors shadow-md"
          >
            Upload File
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOrders.map((order) => (
            <div key={order._id} className="bg-white dark:bg-dark-950 border border-slate-200 dark:border-dark-800 rounded-3xl p-5 flex flex-col justify-between hover:border-blue-500/40 dark:hover:border-blue-500/40 shadow-sm hover:shadow-md transition-all group">
              
              <div className="space-y-4">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900 dark:text-white">#{order.orderId}</h3>
                    <span className="text-xs text-slate-500 dark:text-dark-400 font-medium">
                ...
                <div className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 dark:bg-dark-900 border border-slate-200 dark:border-dark-800">
                  <FileText className="h-5 w-5 text-blue-600 dark:text-blue-400 shrink-0" />
                  <div className="overflow-hidden">
                    <h4 className="text-xs font-bold text-slate-900 dark:text-white truncate">{order.documentId?.originalName || 'Deleted File'}</h4>
                    <p className="text-xs text-slate-500 dark:text-dark-400 capitalize font-medium">{order.printType === 'bw' ? 'Black & White' : 'Color'} print</p>
                  </div>
                </div>
              ...
              <div className="mt-5 pt-4 border-t border-slate-150 dark:border-dark-850 flex justify-end">
                <Link
                  to={`/orders/${order._id}`}
                  className="flex items-center gap-1.5 text-xs text-blue-600 dark:text-blue-400 font-semibold group-hover:text-blue-700 dark:group-hover:text-blue-300 transition-colors"
                >
